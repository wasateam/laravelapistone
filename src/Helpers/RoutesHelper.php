...
use Wasateam\Laravelapistone\Controllers\ShowcaseController;
...
class RoutesHelper
{
  ...
  public static function cms_admin_routes()
  {
    ...
    if (config('stone.showcase')) {
      Route::get('/showcase/order', [ShowcaseController::class, 'order_get']);
      Route::patch('/showcase/order', [ShowcaseController::class, 'order_patch']);
      Route::resource('showcase', ShowcaseController::class)->only([
        'index', 'show', 'store', 'update', 'destroy',
      ])->shallow();
    }
  }
  ...
  public static function webapi_public_routes()
  {
    ...
    if (config('stone.shop')) {
      ...
      Route::post('/shop_cart_product', [ShopCartProductController::class, 'store'])->middleware('throttle:60,1');
      ...
      //   'delete',
      // ])->shallow()->middleware('throttle:10,1');
      Route::post('/auth/shop_cart_product/{shop_cart_product_id}/update', [ShopCartProductController::class, 'update_auth_cart_product'])->middleware('throttle:60,1');
    ...
    # Google
    if (config('stone.google')) {
      if (config('stone.google.merchant')) {
        if (config('stone.google.merchant.products')) {
          if (config('stone.google.merchant.products.url_upload')) {
            Route::get('google/merchant/products/url_upload/shop_product', [GoogleController::class, 'merchant_products_url_upload_shop_product']);
          }
        }
      }
    }
