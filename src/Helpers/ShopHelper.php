...
class ShopHelper
{
  ...
  public static function getOrderProductsAmount($shop_order_shop_products)
  {
    ...
    foreach ($shop_order_shop_products as $shop_order_shop_product) {
      $count = $shop_order_shop_product['count'];
      $price = 0;
      if (isset($shop_order_shop_product->shop_order_shop_product_spec)) {
        $price = $shop_order_shop_product->shop_order_shop_product_spec->shop_product_spec['discount_price'] && config('stone.shop.discount_price') ? $shop_order_shop_product->shop_order_shop_product_spec->shop_product_spec['discount_price'] : $shop_order_shop_product->shop_order_shop_product_spec->shop_product_spec['price'];
      } else {
        if ($shop_order_shop_product->shop_product) {
          $price = $shop_order_shop_product->shop_product['discount_price'] && config('stone.shop.discount_price') ? $shop_order_shop_product->shop_product['discount_price'] : $shop_order_shop_product->shop_product['price'];
        } else {
  ...
  public static function getShopOrderArrayForExport($shop_orders, $request)
  {
    ...
    foreach ($shop_orders as $shop_order) {
      ...
      foreach ($order_products as $index => $order_product) {
        ...
        $invite_no                          = '';
        $invite_no_deduct                   = '';
        $shop_campaign_order_type_deduct    = '0';
        ...
        if ($index == 0) {
          ...
          $product_price_cost                 = $order_product->cost ? $order_product->cost : '0';
          $original_count                     = $order_product->original_count ? $order_product->original_count : '0';
          $subtotal_price                     = $product_price * $order_product->original_count ? $product_price * $order_product->original_count : '0';
          $subtotal_cost                      = $order_product->cost * $order_product->original_count ? $order_product->cost * $order_product->original_count : '0';
          $order_price                        = $shop_order->order_price ? $shop_order->order_price : '0';
          $order_cost_products                = $shop_order->order_cost_products ? $shop_order->order_cost_products : '0';
          $freight                            = $shop_order->freight ? $shop_order->freight : '0';
          $freight_deduct                     = $shop_order->freight_deduct ? $shop_order->freight_deduct : '0';
          $bonus_points_deduct                = $shop_order->bonus_points_deduct ? $shop_order->bonus_points_deduct : '0';
          $shop_campaign_discount_code_deduct = $shop_order->shop_campaign_discount_code_deduct ? $shop_order->shop_campaign_discount_code_deduct : '0';
          ...
          $invite_no_deduct                   = $shop_order->invite_no_deduct ? $shop_order->invite_no_deduct : '0';
          $order_price                        = $shop_order->order_price ? $shop_order->order_price : '0';
          $customer_service_remark            = $shop_order->customer_service_remark;
          $pay_type_text                      = $shop_order->pay_type_text;
          $return_count                       = ShopHelper::getProductReturnCount($order_product) ? ShopHelper::getProductReturnCount($order_product) : '0';
          $return_subtotal_price              = $return_count * $order_product->price ? $return_count * $order_product->price : '0';
          $return_subtotal_cost               = $return_count * $order_product->cost ? $return_count * $order_product->cost : '0';
          $return_price                       = $shop_order->return_price ? $shop_order->return_price : '0';
          $return_cost                        = $shop_order->return_cost ? $shop_order->return_cost : '0';
          $return_reason                      = $shop_order->return_reason;
          $order_price_after_return           = $shop_order->order_price_after_return;
          $order_price_after_return           = $shop_order->order_price_after_return;
          $order_price_after_return           = $shop_order->order_price_after_return;
          $order_cost                         = $shop_order->order_cost ? $shop_order->order_cost : '0';
        } else {
          $shop_product_no       = $shop_product ? $shop_product->no : '';
          $shop_product_name     = $order_product->name;
          $shop_product_spec     = $order_product->spec;
